...
这个类是一个非常标准的抽象类，将基础的数据全部处理掉，然后在 MASConstraint+Private.h 中使用分类 MASConstraint (Abstract) 和一个匿名分类来声明几个基类和子类都将使用到的方法，并且在这里声明代理和代理方法。这一段很有意思，对于开发者来说应该能提供很多灵感。使用分类，能帮你减少很多代码。
...
2. 巧用分类：使用分类，配合继承和代理，可以简化很多代码，如果说链式语法在实战中使用还是比较麻烦，那么熟练使用分类、继承，将会让你的代码更上一层楼。
